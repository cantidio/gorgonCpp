...
//				(*this)[getSize()-1].setType(pImageSheet.getType());
	...
	)
	{
		...
		(
			...
			&& pImage.getPixel(Core::Point(current.getX(),current.getY() - 1)) != pTransparentColor
...
//			printf("baixo\n");
		...
		(
			...
			&& pImage.getPixel(Core::Point(current.getX() + 1,current.getY())) != pTransparentColor
...
//			printf("esquerda\n");
		...
		(
			current.getY() - 1 > pInit.getY()
			&& current.getX() - 1 > -1
			&& pImage.getPixel(Core::Point(current.getX() - 1,current.getY() - 1)) != pTransparentColor
...
//			printf("direita-baixo\n");
	...
	)
	{
		...
		sprite.blitImage(pImage,Core::Point(0,0),Core::Point(pPosX,pPosY),width,height);
		sprite.updateBuffer();
		if(!sprite.isEmpty())
		{
			add(sprite);
		...
		(
			Core::Point(pPosX, pPosY),
			Core::Point(pPosX + width, pPosY + height),
			pBackgroundColor,
			true
		);
		return width;
